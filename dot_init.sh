...
[[ -d ".BUILDS/yay" ]] && git clone https://aur.archlinux.org/yay.git .BUILDS/yay && pushd .BUILDS/yay && makepkg -si --noconfirm
...
[[ ! -d ".BUILDS/python" ]] && pushd ~/.BUILDS && wget https://www.python.org/ftp/python/3.12.0/Python-3.12.0.tar.xz
